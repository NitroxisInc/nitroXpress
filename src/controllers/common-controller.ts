...
export class Controller {
  ...
  private login = asyncHandler(async function (req: express.Request, res: express.Response) {
  ...
  private doLogin = [
    [
      body("email", myLanguage.invalidEmail).isEmail(),
      body("password", myLanguage.invalidPassword)
    ],
    asyncHandler(async (req: express.Request, res: express.Response, next: express.NextFunction) => {
      ...
      try {
        ...
        req.session.messages = [
          ...(req.session.messages || []),
          myLanguage.successfullyLoggedin
        ]
        ...
        req.session.errors = [
          ...(req.session.errors || []),
          myLanguage.wrongUserCredentials
        ]
        req.session.save(() => {
          res.redirect("/")
        })
